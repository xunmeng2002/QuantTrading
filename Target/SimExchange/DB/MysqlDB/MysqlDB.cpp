...
{
	auto start = steady_clock::now();
	const char* sql = "CREATE TABLE IF NOT EXISTS t_AccountLoginSession(`AccountID` char(32), `SessionID` bigint, `IPAddress` char(32), INDEX AccountLoginSessionAccountID(AccountID), PRIMARY KEY(SessionID)) ENGINE=MyISAM DEFAULT COLLATE='utf8mb4_bin';";
	if (m_AccountLoginSessionCreateStatement == nullptr)
	{
		m_AccountLoginSessionCreateStatement = m_DBConnection->prepareStatement(sql);
	}
...
{
	auto start = steady_clock::now();
	const char* sql = "DROP TABLE IF EXISTS t_AccountLoginSession;";
	if (m_AccountLoginSessionDropStatement == nullptr)
	{
		m_AccountLoginSessionDropStatement = m_DBConnection->prepareStatement("sql");
	}
