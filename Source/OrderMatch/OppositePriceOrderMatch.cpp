...
OppositePriceOrderMatch::OppositePriceOrderMatch(mdb::Mdb* mdb, const DateType& tradingDay, int maxTradeID)
...
{
	if (order->Direction == DirectionType::Buy && DoubleLess(order->Price, price))
		return false;
	else if (order->Direction == DirectionType::Sell && DoubleGreat(order->Price, price))
		return false;
