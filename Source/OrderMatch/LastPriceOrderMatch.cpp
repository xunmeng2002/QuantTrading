...
LastPriceOrderMatch::LastPriceOrderMatch(mdb::Mdb* mdb, const DateType& tradingDay, int maxTradeID)
...
{
	if (order->Direction == DirectionType::Buy && DoubleLess(order->Price, mdTick->LastPrice))
		return false;
	else if (order->Direction == DirectionType::Sell && DoubleLess(order->Price, mdTick->LastPrice))
		return false;
