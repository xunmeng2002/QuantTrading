...
- 条目 ID：`D.*` = 原 ✅ 已完成（`D.00`–`D.48` 已归档）；`Q.*` = 原 ❓ 待讨论；`R.*` = 原 🔄 进行中。ID 取自 2026-09-13 拆分时的文档顺序，此后新归档条目按 `D.49`、`D.50`… 递增。
- **批次号 → 归档 ID 对照**（主文件正文里出现"第 N 批"时按此换算）：第二批 `D.32`(2026-09-10) / `D.35`(2026-09-12)、第三批 `D.36`、第四批 `D.37`、第五批 `D.38`、第六批 `D.40`、第七批 `D.41`、第八批 `D.42`、第九批 `D.43`、第十批 `D.45`、第十一批 `D.46`、第十二批 `D.47`、第十三批 `D.48`(2026-09-17)。
- 必须查归档的时机：引用 2026-09-12 及更早的结论时；复审某个已关闭事项时；主文件某条写明"见归档 `Q.xx` / `R.xx`"时。
- 滚动规矩：已完成区超过 5 批时，把最旧的整条**原样**移入归档（主文件目标 ≤ 50 KB）。

## ✅ 已完成

> 更早的 49 条（`D.00`–`D.48`，2026-08-08 ~ 2026-09-17）已归档，见 [`PROGRESS-archive.md`](PROGRESS-archive.md)。已滚动六次：第九批（`D.43`）、分层归档条目（`D.44`）、第十批（`D.45`）、第十一批（`D.46`）、第十二批（`D.47`）（以上 2026-09-17 搬入）、第十三批（`D.48`，2026-09-18 搬入）。
...
- **2026-09-18（第十五批）回测运行契约 + 手续费全链路分列 + 基本数据 InitDb（A3 / A4 / A5 一次性落地）**：
  ...
  - **测试 I/O 示例（§7）**：单测用例 1 的输入输出——
    ...
    ```
    端到端验收（需用户在 VS 构建后执行）：`cd bin/Release && python ../test/PythonStrategyGrid/grid_strategy.py ; echo $?` —— 期望退出码 `0`、产物目录新增 `t_BaseCommission.csv`(6 行)/`t_CommissionGroup.csv`(1 行)、`t_Trade.csv` 的 `Commission` 列被封底到 5.0（种子 `MinCommission = 5` 且每笔成交 1 手），且 `t_Capital.csv` 末行满足 `Balance == PreBalance + 盈亏 − Commission − StampTax − TransferFee`。
...
- **回测平台化（2026-09-17 起；引擎侧 A3/A4/A5 已交付，见 ✅ 第十五批）**：形态、RunId 归属、产物布局三条已定案（见归档 `D.48`），平台化的三块硬骨头已于 2026-09-18 一次性落地，**剩余全在调度侧**：
  ...
  4. **未决**：调度层跑在本机还是云上；产物是否上传对象存储（SQLite 不能 range-read、且需要本地文件系统与可靠文件锁，Parquet 可以）；并发上限由谁定（每个回测进程都要读 parquet、建 mdb、跑撮合，CPU 密集）。
