...
{
    // bar 周期聚合器：把输入周期 bar（精度由数据集决定，如 1m/5m）聚合成目标周期 bar。
    // 桶边界锚定在「交易节」段首（注入的 TradeSessions，由所属环境的 SessionFile 装载，跟交易所与品种走）：
    ...
    public:
        ...
        // 输入精度能否聚合到目标精度：不能时 WriteLog(Error) 并抛 std::logic_error。
        // 首根输入校验（ValidateInputBar）与引擎装载期预校验共用同一判据，避免两处规则漂移。
        // instrumentID 仅用于日志定位（无合约上下文时可传空串）。
    ...
    private:
        ...
        // 值 nullptr = 该合约无交易节，回落墙钟对齐。缓存的裸指针指向注入的 m_TradeSessions，
        // 该实例由调用方持有且长于本对象，装载完成后内容不再变化（TradeSessions 拒绝重复装载），故缓存不设失效钩子；
