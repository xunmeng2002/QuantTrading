...
TEST_CASE("GridStrategy anchors ladder from first bar when replay delivers bars instead of ticks")
{
    ...
    REQUIRE(fake_api.InsertRequests.size() == 4);
    CHECK(fake_api.InsertRequests[0].Direction == DirectionType::Buy);
    CHECK(fake_api.InsertRequests[0].Price == doctest::Approx(3990.0));
    CHECK(fake_api.InsertRequests[1].Direction == DirectionType::Sell);
    CHECK(fake_api.InsertRequests[1].Price == doctest::Approx(4010.0));

    auto next_bar = MakeMdBarField("IF2503", 202410010940LL, 4050.0);
    fake_api.RegisteredSpi->OnRtnBarMarketData(&next_bar);
...
TEST_CASE("GridStrategy places close for filled volume when partially filled open order is canceled")
{
    ...
    fake_api.RegisteredSpi->OnRtnOrder(&open_order);
    ...
    fake_api.RegisteredSpi->OnRtnOrder(&close_order);
...
TEST_CASE("GridStrategy re-places close order for remaining volume when close order is canceled")
{
    ...
    fake_api.RegisteredSpi->OnRtnTrade(&open_trade);
    REQUIRE(fake_api.InsertRequests.size() == 5);
    CHECK(fake_api.InsertRequests[4].Volume == 2);
    ...
    fake_api.RegisteredSpi->OnRtnOrder(&canceled_close_order);
    ...
    fake_api.RegisteredSpi->OnRtnOrder(&replaced_close_order);
    auto replaced_close_trade = MakeTradeField("IF2503", 44, DirectionType::Sell, OffsetFlagType::Close, 4000.0, 1, 300, 8.5);
    fake_api.RegisteredSpi->OnRtnTrade(&replaced_close_trade);
    CHECK(strategy.GetLongPosition("IF2503") == 0);
}
...
TEST_CASE("GridStrategy re-places close after day-end cancel when slot cycles through completed pair")
{
    ...
    fake_api.RegisteredSpi->OnRtnTrade(&open_trade2);
...
TEST_CASE("GridStrategy full day-end cancel sweep resets all open slots and re-places ladder next day")
{
    ...
    for (int clientOrderId = 1; clientOrderId <= 4; ++clientOrderId)
    {
        auto canceled_order = MakeCanceledOrderField("IF2503", 100 + clientOrderId, clientOrderId, OrderStatusType::Canceled, 0);
        fake_api.RegisteredSpi->OnRtnOrder(&canceled_order);
    }
    CHECK(fake_api.InsertRequests.size() == 4);

    SessionEndField session_end;
    fake_api.RegisteredSpi->OnRtnSessionEnd(&session_end);
    SessionBeginField session_begin;
    fake_api.RegisteredSpi->OnRtnSessionBegin(&session_begin);
