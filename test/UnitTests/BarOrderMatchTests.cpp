...
TEST_CASE("限价买低于bar最低价不成交直到bar触及")
{
    ...
    order_match.OnBar(MakeBarRecord(bar_pool, MakeUpdateTs(20240301, 90200, 0), 96.5, 97.0, 96.0, 96.5));
    REQUIRE(subscriber.trades.size() == 1);
...
TEST_CASE("限价卖低于bar最低价按最低价成交且高于最高价不成交")
{
    ...
    order_match.OnBar(MakeBarRecord(bar_pool, MakeUpdateTs(20240301, 90200, 0), 102.0, 106.0, 102.0, 105.5));
