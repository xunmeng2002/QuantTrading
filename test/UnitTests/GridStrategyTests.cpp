...
TEST_CASE("GridStrategy places ladder around first tick anchor")
{
    ...
    CHECK(fake_api.InsertRequests[0].OffsetFlag == OffsetFlagType::Open);
    CHECK(fake_api.InsertRequests[0].Price == doctest::Approx(3990.0));
    CHECK(fake_api.InsertRequests[0].ClientOrderId == 1);
    ...
    CHECK(fake_api.InsertRequests[2].Price == doctest::Approx(3980.0));
    CHECK(fake_api.InsertRequests[3].Direction == DirectionType::Sell);
    CHECK(fake_api.InsertRequests[3].Price == doctest::Approx(4020.0));
    for (auto& insert_request : fake_api.InsertRequests)
    {
...
TEST_CASE("GridStrategy places close order once after open fill and ignores duplicate fill")
{
    ...
    fake_api.RegisteredSpi->OnRtnTrade(&open_trade);
    CHECK(fake_api.InsertRequests.size() == 5);
}
...
TEST_CASE("GridStrategy re-places rejected level at next session anchor")
{
    ...
    fake_api.RegisteredSpi->OnRspInsertOrder(&rejected_request, &reject_info, 0, true);
